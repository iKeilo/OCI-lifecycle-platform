...
Docker 版适合把前端、Go 后端和运行时依赖打成一个镜像部署。默认使用容器内 Go 服务托管前端静态文件，不依赖 nginx。
...
```bash
sudo WEB_PORT=8088 bash scripts/docker-install.sh install
```
...
sudo PANEL_PASSWORD='change-this-password' \
  WEB_PORT=18080 \
  bash scripts/docker-install.sh install
```
...
```bash
sudo bash scripts/docker-install.sh configure-oci
```
...
```bash
sudo bash scripts/docker-install.sh update
```
...
```bash
sudo bash scripts/docker-install.sh backup
```
...
```bash
sudo bash scripts/docker-install.sh uninstall
```
