...
```json
{
  ...
  "note": "reinstall from console"
}
```

行为：
- 仅在 `OCI_EXECUTION_MODE=oci` 下创建真实任务；local executor 不会冒充重装成功。
- 创建 `重装系统` Job，`operation=reinstall`。
- OCI executor 调用 Compute `UpdateInstance`，使用 `SourceDetails: UpdateInstanceSourceViaImageDetails`。
- 支持 `imageId`、`preserveOldBootVolume`、`bootVolumeSizeGb`。
- `bootVolumeVpusPerGb` 通过后续启动盘更新链路调整。
- 任务创建成功后必须创建站内通知，并尝试按邮件配置投递给管理员收件人。
- OCI 执行成功后必须创建“重装系统成功”站内通知，并尝试邮件投递。
- OCI 执行失败后必须创建“重装系统失败”站内通知，并尝试邮件投递；通知中保留真实错误码和错误信息。
- 重装通知包含操作、Job ID、操作人、实例名称、实例 OCID、Profile、Region、Compartment、镜像、启动盘大小、启动盘性能、是否保留旧启动盘、公网 IPv4、公网 IPv6、私网 IP、OCI request id / work request id。
- 重装通知中的 SSH 密码字段必须明确写为“未生成 / 未变更”。当前重装路径不能生成或注入新 root 密码，不能在通知里伪造密码。
- 当前不支持重装时注入 root 密码、cloud-init 或 SSH key。OCI 文档限制已启动实例不能修改 `user_data` 和 `ssh_authorized_keys`；请求中包含这些字段会返回 `REINSTALL_PASSWORD_INJECTION_UNSUPPORTED` 或 Store validation error。
- 已有实例“重置密码”功能已移除；当前版本不提供 `/system/password-reset` API，也不规划通过 Agent 或离线救援盘实现。
...
重装系统通知：
- `任务已创建`、`重装成功`、`重装失败` 三个阶段都会创建站内通知。
- 三个阶段都设置 `emailRequested=true`；如果邮件服务启用则发送邮件，如果未启用则通知内记录 `emailError=email delivery is disabled`。
- 通知标记为 `sensitive=true`，因为包含服务器连接信息和 OCI 引用。
- SSH 密码只显示“未生成 / 未变更”，除非未来有真实验证过的密码重置或注入路径。
